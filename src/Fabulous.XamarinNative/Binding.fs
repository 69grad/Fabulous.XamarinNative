// Copyright 2018 Fabulous contributors. See https://github.com/fsprojects/Fabulous/blob/master/LICENSE.md for license.
namespace Fabulous.XamarinNative

type internal Getter<'model> = 'model -> obj

type internal Setter<'model, 'msg> = obj -> 'model -> 'msg (*Value*)

type internal Execute<'model, 'msg> = obj -> 'model -> 'msg (*CommandParameter*)

type internal CanExecute<'model> = obj -> 'model -> bool (*CommandParameter*)

type internal ValidSetter<'model, 'msg> = obj -> 'model -> Result<'msg, string> (*Value*)

type ViewBinding<'model, 'msg> = string * ViewVariable<'model, 'msg>
...
and ViewVariable<'model, 'msg> =
    ...
    | BindMap of Getter<'model> * (obj -> obj) (* '_a *)

and ViewSubModel<'model, 'msg> =
    // representation is internal
    internal
    | ViewSubModel of (obj -> unit) * string * ('model -> obj) * (obj -> 'msg) (* '_msg *)  * ViewBindings<obj, obj>
    member x.Name =
        (let (ViewSubModel(_, nm, _, _, _)) = x in nm)
...
module Binding =
    ...
    let private boxVariable (variable: ViewVariable<'model, 'msg>): ViewVariable<obj, obj> =
        ...
        | BindTwoWayValidation(getter, setter) ->
            BindTwoWayValidation(unbox >> getter, (fun v m -> setter v (unbox m) |> Result.map box))
        | BindCmd(exec, canExec) ->
            BindCmd((fun p m -> exec p (unbox m) |> box), (fun p m -> canExec p (unbox m)))
        | BindSubModel(ViewSubModel(page, name, getter, toMsg, bindings)) ->
            BindSubModel(ViewSubModel(page, name, unbox >> getter, toMsg >> box, bindings))
    ...
    ///<param name="name">Binding name</param>
    let subView init (getter: 'model -> '_model) (toMsg: '_msg -> 'msg) (viewBindings: ViewBindings<'_model, '_msg>)
        name: ViewBinding<'model, 'msg> =
